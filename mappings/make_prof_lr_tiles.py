...
for f_path in forward_files:
    ...
    ad_path = os.path.join(run_dir + 'profiles/', f"ad{fname}.{tile_suffix}.equi.nc")
    ...
    if os.path.exists(ad_path):
        ...
        rename_dict = {v: f"ad{v}" for v in ds_ad.data_vars if v != 'prof_ind_glob'}
        ds_ad = ds_ad.rename(rename_dict)

        # Merge on iPROF (since they share local indexing within the same high-res tile)
        # We ensure they align on prof_ind_glob
        combined_tile = xr.merge([ds_fwd, ds_ad.drop_vars('prof_ind_glob', errors='ignore')])
    else:
    ...
    combined_tile = combined_tile.assign_coords(iPROF=combined_tile.prof_ind_glob.values.astype(int))
...
hr_all['prof_LR_tile'] = (['iPROF'], np.array([lr_ocean_map.get(t, -999) for t in assigned_tile_id]))
...
for tile_num, tile_ds in grouped:
    ...
    # --- Write Forward File ---
    fwd_out = tile_ds[vars_to_keep].sortby('prof_ind_glob')
    fwd_out.to_netcdf(os.path.join(out_dir, f"{fname}.{tile_str}.equi.nc"), format='NETCDF4_CLASSIC')

    # --- Write Adjoint File ---
    ad_vars_to_extract = [f"ad{v}" for v in vars_to_keep if v != 'prof_ind_glob']
    rename_map = {f"ad{v}": v for v in vars_to_keep if v != 'prof_ind_glob'}

    ad_out = tile_ds[['prof_ind_glob'] + ad_vars_to_extract].rename(rename_map)
    ad_out = ad_out.sortby('prof_ind_glob')
