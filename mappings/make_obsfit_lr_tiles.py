...
fname = 'swot_obsfit_cycle9_labsea_LONcorrected_removecoast_400m'
fnames = [fname, 'ad'+fname]

iter = sys.argv[1]
ext = sys.argv[2]

run_dir = '/scratch/shoshi/labsea_MG_12/assim_swot_argo/run_adhi_it' + iter + '_' + ext + '/'
run_dir_lo = '/scratch/shoshi/labsea_MG_12/assim_swot_argo/run_adlo_it' + iter + '_' + ext + '/'
...
def get_hr_ocean_files(run_dir, f, ocean_mapping, lr_ocean_tile):
    ...
    for tile in hr_tiles:
        file_path = os.path.join(run_dir, f"{f}.{tile:03d}.001.equi.nc")
...
def combine_equi_files(files):
    ...
    # Concatenate along iSAMPLE (even if lengths differ)
    equi = xr.concat(datasets, dim='iSAMPLE', combine_attrs='override')

    # Rebuild a continuous iSAMPLE coordinate
    equi = equi.assign_coords(iSAMPLE=range(equi.dims['iSAMPLE']))
    equi = equi.sortby('sample_ind_glob')

    return equi


out_dir = os.path.join(run_dir, 'swot_LR_equi')
